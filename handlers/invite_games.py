...
async def process_invite_xo_game(callback: CallbackQuery, db, translator):
    ...
    user_id_one_message = await callback.message.edit_text(translator.get('send_request_for_game'),
                                                           reply_markup=keyboard_before_cancel_game_inline)

    user_id_two_message = await callback.bot.send_message(text=translator.get('get_game_request_xo'),
                                                          chat_id=user_id_two,
                                                          reply_markup=keyboard_before_accept_game_inline)

    await db.set_user_state(user_id_one, "waiting_for_start",
                            json.dumps({"message_id": user_id_one_message.message_id}))
    await db.set_user_state(user_id_two, "waiting_for_start",
                            json.dumps({"message_id": user_id_two_message.message_id}))

    logging.info(
        f"user message one: {user_id_one_message.message_id}, user message two: {user_id_two_message.message_id}")
...
async def process_accept_xo_game(callback: CallbackQuery, db, translator):
    ...
    await callback.message.edit_text(translator.get('start_game_go_x'),
                                     reply_markup=game.get_board_markup())

    await callback.bot.edit_message_text(
        chat_id=user_id_two,
        message_id=user_two_message_id,
        text=translator.get('start_game_go_x'),
        reply_markup=game.get_board_markup()
    )
